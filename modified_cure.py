...
from modified_kdtree import kdtree
...
class cure:
    ...
    def __merge_clusters(self, cluster1, cluster2):
        ...
        if merged_cluster.points[1:] == merged_cluster.points[:-1]:
            ...
            # data = []
            # data.append(cluster1.points[0])
            # data.append(cluster2.points[0])
            bary = dtw_barycenter_averaging(merged_cluster.points, max_iter=50)
            dimension = len(bary[0])
            merged_cluster.mean = [0] * dimension
            for i in range(0, dimension):
                merged_cluster.mean[i] = bary[0][i][0]
            ##################################################################################
        ...
        for point in temporary:
            ...
            #Changes made by Meet
            merged_cluster_len = len(merged_cluster.mean)
            point_len = len(point)
            if merged_cluster_len > point_len:
                num = merged_cluster_len - point_len
                for i in range(num):
                    point.append(0.0)
            else:
                num = point_len - merged_cluster_len
                for i in range(num):
                    merged_cluster.mean.append(0.0)

            series = compute_dtw_series(merged_cluster.mean, point)
            temp_dist = [0] * len(series)
            for i in range(len(series)):
                temp_dist[i] = self.__compression * series[i]

            dim = max(len(temp_dist), len(point))
            temp_len = len(temp_dist)
            point_len = len(point)
            representative_point = [0] * dim
            if temp_len > point_len:
                num = temp_len - point_len
                for i in range(num):
                    point.append(0.0)
            else:
                num = point_len - temp_len
                for i in range(num):
                    temp_dist.append(0.0)
            for i in range(dim):
                representative_point[i] = temp_dist[i] + point[i]
